...
find the Largest Connected Component (LCC), and sample 10 nodes.

Generates two output files:
1. An analysis file with graph statistics (*_analysis.txt).
2. A source node file with 10 sampled nodes (*_sources.txt), one node per line.
"""
...
def analyze_graph(row_file, col_file, output_base_path):
    ...
    # 4. Randomly pick 10 nodes from the LCC
    num_samples = 10
    ...
    sources_file = Path(output_base_path).parent / f"{Path(output_base_path).stem}_sources.txt"

    with open(sources_file, 'w') as out:
        out.write(f"# 10 Random Source Nodes (0-indexed) from LCC of {Path(output_base_path).stem}\n")
        out.write(f"# LCC Size: {lcc_size}\n")
        # Write nodes, one per line, for easy script reading
        for node in random_nodes:
            out.write(f"{node}\n")

    print("\n⭐ 10 Random Nodes Sampled from LCC (Saved for script use):")
...
def main():
    parser = argparse.ArgumentParser(
        description="Load CSR graph, find LCC, and sample 10 nodes (stdlib only), saving to two separate output files."
    )
    ...
    parser.add_argument("--base-out", required=True, help="Base output path (e.g., path/to/dataset_name) used to generate *_analysis.txt and *_sources.txt")
    args = parser.parse_args()

    analyze_graph(args.row, args.col, args.base_out)
