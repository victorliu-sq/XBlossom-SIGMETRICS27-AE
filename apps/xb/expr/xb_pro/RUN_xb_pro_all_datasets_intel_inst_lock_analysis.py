...
# ============================================================
# Extract total lock loads
# ============================================================

def extract_total_lock_loads(prof_file):
    """
    From perf line like:
    439068687,,cpu_core/mem_inst_retired.lock_loads/,...
    """
    total = 0
    with open(prof_file) as f:
        for line in f:
...
if len(sys.argv) != 5:
    print("Usage: RUN_xb_pro_all_datasets_intel_inst_analysis.py <dataset> <timing_file> <prof_file> <summary_csv>")
    sys.exit(1)
...
total_lock_loads   = extract_total_lock_loads(prof_file)

# XB-Pro executed ROUNDS=10 runs → total runtime is:
total_runtime_s = avg_runtime_s * 10

# Compute derived metrics
inst_exec_rate_gips = (total_instructions / total_runtime_s) / 1e9
lock_load_rate_mps  = (total_lock_loads / total_runtime_s) / 1e6   # Mlocks/s
...
print(
    ...
    f"Mlocks/s={lock_load_rate_mps:.4f}"
)
