...
METRICS_DIR="tmp/bfs_gunrock_time"
...
PYTHON_SCRIPT="expr/experiments/figure_6/RUN_bfs_gunrock_time.py"
...
# 1. Dataset Name
# 2. Graph Path (.mtx)
# 3. Source Nodes File (*_sources.txt)
# ------------------------------------------------------------
DATASETS=(
...
do
  ...
  TIMING_OUT="$METRICS_DIR/bfs_gunrock_timing_${DATASET_NAME}.txt"

  # Load source nodes, skip comments
  mapfile -t SRC_NODES < <(grep -v '^#' "$SRC_FILE_PATH")
  SRC=${SRC_NODES[0]}
#  PROFILING_OUT_1="$METRICS_DIR/bfs_gunrock_profile_${DATASET_NAME}_1.csv"
#  PROFILING_OUT_2="$METRICS_DIR/bfs_gunrock_profile_${DATASET_NAME}_2.csv"

  # Step 1: Measure the runtime for one round
  echo "  Running Gunrock BFS on $DATASET_NAME (src=$SRC)..."
  touch $TIMING_OUT
...
#      $CUDA_BIN -s 0 -m $DATASET_PATH > $PROFILING_OUT_2
  ...
  conda run -n xb-env --no-capture-output python3 "$PYTHON_SCRIPT" "$DATASET_NAME" "$TIMING_OUT" "$SUMMARY_CSV_TEMP"
done
