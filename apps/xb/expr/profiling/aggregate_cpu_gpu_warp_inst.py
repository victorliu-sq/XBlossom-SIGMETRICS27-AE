...
with open(gpu_inst_txt) as f:
    ...
    for line in f:
        ...
        if line.startswith("DATASET:"):
            current_ds = normalize_name(line.split(":")[1].strip().lower())
        elif current_ds and line.startswith("Total Instructions Executed:"):
...
with open(output_csv, "w", newline="") as csvfile:
    ...
    writer.writerow([
        ...
        "gpu_warp_insts", "gpu_time_s", "gpu_thread_ops_per_s"
    ])
    ...
    for ds in sorted(all_datasets):
        ...
        g_warp_insts = gpu_insts.get(ds, 0)
        g_time = gpu_times.get(ds, 0.0)
        # warp instructions × 32 threads
        g_ops_per_s = (g_warp_insts * 32) / g_time if g_time > 0 else 0

        writer.writerow([
            ds,
            c_insts, f"{c_time:.3f}", f"{c_ops_per_s:.2e}",
            g_warp_insts, f"{g_time:.3f}", f"{g_ops_per_s:.2e}"
        ])