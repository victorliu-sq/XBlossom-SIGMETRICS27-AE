...
namespace xblossom {
  void testParBlossom_pro_pernode_na(Graph &G, std::vector<int> &M, int threshold) {
    ...
    for (int i = 0; i < iteration; i++) {
      ...
      parFindMaximumMatchingNoRecursionUpdatePathTable_pro_pernode_na(G, M, valid_M, threshold);
