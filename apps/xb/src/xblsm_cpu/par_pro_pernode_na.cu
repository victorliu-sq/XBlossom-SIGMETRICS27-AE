...
namespace xblossom {
  ...
  void parFindMaximumMatchingNoRecursionUpdatePathTable_pro_pernode_na(Graph &G, std::vector<int> &M, bool &valid_M,
                                                                int threshold) {
    bool finished = false;
    ...
    while (!finished) {
      ...
      parFindAugmentingPathNoRecursionUpdatePathTable_pernode_na(G, M, path_collection, path_table_vector, is_even,
                                                              belongs);
