...
namespace xblossom {
  ...
  void parFindMaximumMatchingNoRecursionUpdatePathTable_pro_2(Graph &G, std::vector<int> &M, bool &valid_M,
    ...
    while (!finished) {
      ...
      // Already put in parFindAugmentingPathNoRecursionUpdatePathTable_pro_2
      // for (auto &sub_vector: path_table_vector) {
      //   sub_vector.clear();
      // }

      // 2. Run the XBlossom algorithm to find disjoint augmenting path
      parFindAugmentingPathNoRecursionUpdatePathTable_pro_2(G, M, path_collection, path_table_vector, is_even, belongs);
  ...
  void parAugmentingPath_pro_2(const std::vector<int> &rowOffsets,
  ...
  void parExpand_pro_2(const std::vector<int> &rowOffsets,
                       const std::vector<int> &columnIndices,
                       const std::vector<int> &nodes_vector,
                       int index, int num_threads, std::vector<int> &is_even, std::vector<int> &belongs,
  ...
  void parBlossom_pro_2(const std::vector<int> &rowOffsets,
                        const std::vector<int> &columnIndices,
                        const std::vector<int> &nodes_vector,
                        int index, int num_threads, std::vector<int> &is_even, std::vector<int> &belongs,
  ...
  void parFindAugmentingPathNoRecursionUpdatePathTable_pro_2(Graph &G,
    ...
    while (!check_all) {
      ...
      for (int begin = 0; begin < num_of_threads; begin++) {
        threads.emplace_back(parAugmentingPath_pro_2,
      ...
      for (int begin = 0; begin < num_of_threads; begin++) {
        threads.emplace_back(parExpand_pro_2,
                             std::ref(G.rowOffsets),
                             std::ref(G.columnIndices),
                             std::ref(nodes_vector),
                             begin,
      ...
      for (int begin = 0; begin < num_of_threads; begin++) {
        threads.emplace_back(parBlossom_pro_2, std::ref(G.rowOffsets), std::ref(G.columnIndices),
                             std::ref(nodes_vector),
