...
namespace xblossom {
  ...
  void parFindMaximumMatchingNoRecursionUpdatePathTable_pro_peredge_nb(Graph &G, std::vector<int> &M, bool &valid_M,
                                                                    int &threshold) {
    bool finished = false;
    ...
    while (!finished) {
      ...
      parFindAugmentingPathNoRecursionUpdatePathTable_pro_peredge_nb(G, M, path_collection, path_table_vector, is_even,
                                                                  belongs);
