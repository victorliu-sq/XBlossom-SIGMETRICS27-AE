...
namespace xblossom {
  ...
  void parFindMaximumMatchingNoRecursionUpdatePathTable_pro_peredge_na(Graph &G, std::vector<int> &M, bool &valid_M,
                                                                    int &threshold) {
    bool finished = false;
    ...
    while (!finished) {
      ...
      parFindAugmentingPathNoRecursionUpdatePathTable_pro_peredge_na(G, M, path_collection, path_table_vector, is_even,
                                                                  belongs);
  ...
  static void parExpand_pro_all_3(const std::vector<int> &rowOffsets,
                           const std::vector<int> &columnIndices,
                           const std::vector<int> &nodes_vector,
                           int index, int num_threads, std::vector<int> &is_even, std::vector<int> &belongs,
  ...
  static void parBlossom_pro_all_3(const std::vector<int> &rowOffsets,
                            const std::vector<int> &columnIndices,
                            const std::vector<int> &nodes_vector,
                            int index, int num_threads, std::vector<int> &is_even, std::vector<int> &belongs,
  ...
  void parFindAugmentingPathNoRecursionUpdatePathTable_pro_peredge_na(Graph &G,
    ...
    while (!check_all) {
      ...
      for (int tid = 0; tid < num_of_threads; tid++) {
        ...
        threads.emplace_back(parAugmentingPath_pro_edges_na,
                             std::ref(v_vector),
                             std::ref(w_vector),
                             start_edge, end_edge,
                             std::ref(is_even),
      ...
      for (int tid = 0; tid < num_of_threads; tid++) {
        int start_edge = tid * edges_per_thread;
        int end_edge = std::min(total_edges, (tid + 1) * edges_per_thread);
        threads.emplace_back(parExpand_pro_edges_na,
                             std::ref(v_vector),
                             std::ref(w_vector),
                             start_edge, end_edge,
                             std::ref(is_even),
      ...
      for (int tid = 0; tid < num_of_threads; tid++) {
        ...
        if (start_edge < end_edge) {
          threads.emplace_back(parBlossom_pro_edges_na,
                               std::ref(v_vector),
                               std::ref(w_vector),
                               start_edge, end_edge,
                               std::ref(is_even),
