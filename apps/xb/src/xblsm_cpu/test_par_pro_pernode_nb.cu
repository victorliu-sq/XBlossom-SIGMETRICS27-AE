...
namespace xblossom {
  void testParBlossom_pro_pernode_nb(Graph &G, std::vector<int> &M, int threshold) {
    ...
    for (int i = 0; i < iteration; i++) {
      ...
      parFindMaximumMatchingNoRecursionUpdatePathTable_pro_pernode_nb(G, M, valid_M, threshold);
