...
namespace xblossom {
  ...
  extern std::chrono::microseconds duration_atomic;
  ...
  void parFindMaximumMatchingNoRecursionUpdatePathTable_pro_all_atomic(Graph &G, std::vector<int> &M, bool &valid_M,
                                                              int threshold) {
    bool finished = false;
    ...
    while (!finished) {
      ...
      parFindAugmentingPathNoRecursionUpdatePathTable_pro_all_atomic(G, M, path_collection, path_table_vector, is_even, belongs);
  ...
  void parAugmentingPath_pro_all_atomic(const std::vector<int> &rowOffsets,
  ...
  void parExpand_pro_all_atomic(const std::vector<int> &rowOffsets,
                       const std::vector<int> &columnIndices,
                       const std::vector<int> &nodes_vector,
                       int index, int num_threads, std::vector<int> &is_even, std::vector<int> &belongs,
  ...
  void parBlossom_pro_all_atomic(const std::vector<int> &rowOffsets,
                        const std::vector<int> &columnIndices,
                        const std::vector<int> &nodes_vector,
                        int index, int num_threads, std::vector<int> &is_even, std::vector<int> &belongs,
  ...
  void parFindAugmentingPathNoRecursionUpdatePathTable_pro_all_atomic(Graph &G,
    ...
    // Reuse remaining trees
    for (int x = 0; x < nodes; x++) {
      int mate = M[x]; // current match
      int root = belongs[x]; // tree root of node x (initialized earlier)

      if (mate == -1) {
        // Case 1: Unmatched node
        is_even[x] = 1;
        belongs[x] = x; // root is itself
        vector_1.push_back(x); // enqueue in queue1
        path_table_vector[x].clear();
      } else {
        // Matched node
        if (root == -1 || M[root] != -1) {
          // Case 2: Not in a tree, or in a tree whose root is matched now
          is_even[x] = 0;
          belongs[x] = -1;
          path_table_vector[x].clear();
          // vector_1.push_back(x); // enqueue in queue1
        } else {
          // Case 3: In a tree and root is still unmatched
          // If already marked even, enqueue in queue2
          if (is_even[x]) {
            vector_2.push_back(x); // queue2
          }
          // Odd nodes stay silent
        }
      }
    }
    ...
    while (!check_all) {
      ...
      for (int begin = 0; begin < num_of_threads; begin++) {
        threads.emplace_back(parAugmentingPath_pro_all_atomic,
      ...
      for (int begin = 0; begin < num_of_threads; begin++) {
        threads.emplace_back(parExpand_pro_all_atomic,
                             std::ref(G.rowOffsets),
                             std::ref(G.columnIndices),
                             std::ref(nodes_vector),
                             begin,
      ...
      for (int begin = 0; begin < num_of_threads; begin++) {
        threads.emplace_back(parBlossom_pro_all_atomic, std::ref(G.rowOffsets), std::ref(G.columnIndices),
                             std::ref(nodes_vector),
