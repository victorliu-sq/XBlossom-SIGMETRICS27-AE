#include "xbsm_gpu/xblossom_gpu_engine_ob_4.h"
#include <glog/logging.h>
#include "utils/cuda_algo.h"

namespace zblossom {
  XBlossomGpuEngineOb4::XBlossomGpuEngineOb4(const HGraph &g)
  ...
  XBlossomGpuEngineOb4::XBlossomGpuEngineOb4(const HGraph &g, size_t path_table_buffer_ratio)
    : SecondaryMaximumMatchingGPUEngine(g.nnodes),
      // host_graph_(g),
      device_graph_(g),
      ...
      // For ParBlossom
      // 50 for Hyperlink
      blossom_offset_list_(30 * nnodes_ + 1),
      num_nodes_in_blossom_list_(30 * nnodes_ + 1),
      num_odd_nodes_vside_in_blossom_list_(30 * nnodes_ + 1),
      num_odd_nodes_in_blossom_list_(30 * nnodes_ + 1),
      num_odd_nodes_in_blossom_psum_(30 * nnodes_ + 1) {
  }

  void XBlossomGpuEngineOb4::FindAndFlipAugmentingPath() {
  ...
  void XBlossomGpuEngineOb4::InitAlternatingForest() {
  ...
  bool XBlossomGpuEngineOb4::FindAndFlipAugmentingPathInAlternatingForest() {
  ...
  void XBlossomGpuEngineOb4::ExpandAlternatingForest() {
    auto start_time = getNanoSecond();
  ...
  void XBlossomGpuEngineOb4::TransformOddNodesInBlossom() {
    auto start_time = getNanoSecond();
