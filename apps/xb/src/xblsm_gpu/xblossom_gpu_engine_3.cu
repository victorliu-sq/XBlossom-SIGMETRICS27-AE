#include "xbsm_gpu/xblossom_gpu_engine_3.h"
#include <glog/logging.h>
#include "utils/cuda_algo.h"

namespace zblossom {
  XBlossomGpuEngine3::XBlossomGpuEngine3(const HGraph &g)
  ...
  void XBlossomGpuEngine3::FindAndFlipAugmentingPath() {
  ...
  void XBlossomGpuEngine3::InitAlternatingForest() {
  ...
  bool XBlossomGpuEngine3::FindAndFlipAugmentingPathInAlternatingForest() {
  ...
  void XBlossomGpuEngine3::ExpandAlternatingForest() {
    auto start_time = getNanoSecond();
  ...
  void XBlossomGpuEngine3::TransformOddNodesInBlossom() {
    auto start_time = getNanoSecond();
    ...
    this->ExecuteNTasklet(total_num_pairs, [=] __device__ (index_t tid) mutable {
      ...
      if (is_even_dview[w] && root_v == root_w && matching_dview[v] != w && root_w != nnodes) {
        ...
        // path_table_dview.FindAndAddBlossom(v, w, blossom_offset, nnodes_in_blossom, v_idx_in_blossom);
        ...
        // if (path_table_dview.CheckDuplicationInPath(blossom_offset, nnodes_in_blossom)) {
        //   return;
        // }
