#include "xbsm_gpu/xblossom_gpu_engine_11.h"
...
namespace zblossom {
  XBlossomGpuEngine11::XBlossomGpuEngine11(const HGraph &g)
...
#define ODD_NODE_RATIO 30
  ...
  XBlossomGpuEngine11::XBlossomGpuEngine11(const HGraph &g, size_t path_table_buffer_ratio)
    : SecondaryMaximumMatchingGPUEngine(g.nnodes),
      // host_graph_(g),
      device_graph_(g),
  ...
  void XBlossomGpuEngine11::FindAndFlipAugmentingPath() {
  ...
  void XBlossomGpuEngine11::InitAlternatingForest() {
  ...
  bool XBlossomGpuEngine11::FindAndFlipAugmentingPathInAlternatingForest() {
    ...
    // 1. Copy even nodes from even node queue to frontier array
    auto frontier_dview = this->frontier_.DeviceView();
    this->ExecuteNTasklet(total_even_nodes, [=] __device__ (index_t idx) mutable {
      frontier_dview[idx] = enode_queue_dview[enode_base_offset + idx];
    });

    // 2. sort the frontier nodes
    thrust::sort(thrust::device,
                 frontier_.begin(),
                 frontier_.begin() + total_even_nodes);

    // 3. scan the frontier even nodes
    this->ExecuteNTasklet(total_even_nodes, [=] __device__ (index_t idx) mutable {
      // index_t v = enode_queue_dview[enode_base_offset + idx];
      index_t v = frontier_dview[idx];
    ...
    this->ExecuteNTasklet(total_num_pairs, [=] __device__ (index_t tid) mutable {
      ...
      index_t v = frontier_dview[v_idx];
  ...
  void XBlossomGpuEngine11::ExpandAlternatingForest() {
    auto start_time = getNanoSecond();
  ...
  void XBlossomGpuEngine11::TransformOddNodesInBlossom() {
    auto start_time = getNanoSecond();
