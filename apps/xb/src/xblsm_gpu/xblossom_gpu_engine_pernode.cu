...
#include "xbsm_gpu/xblossom_gpu_engine_pernode.h"

namespace zblossom {
  XBlossomGpuEnginePerNode::XBlossomGpuEnginePerNode(const HGraph &g)
...
#define ODD_NODE_RATIO 30
  ...
  XBlossomGpuEnginePerNode::XBlossomGpuEnginePerNode(const HGraph &g, size_t path_table_buffer_ratio)
    : SecondaryMaximumMatchingGPUEngine(g.nnodes),
      // host_graph_(g),
      device_graph_(g),
  ...
  void XBlossomGpuEnginePerNode::FindAndFlipAugmentingPath() {
  ...
  void XBlossomGpuEnginePerNode::InitAlternatingForest() {
    ...
    this->ExecuteNTasklets(nnodes_, [=] __device__ (index_t x) mutable {
      // With Prev
  ...
  bool XBlossomGpuEnginePerNode::FindAndFlipAugmentingPathInAlternatingForest() {
    ...
    // CUDA_CHECK(cudaDeviceSynchronize());
    //
    // this->ExecuteNTasklets(total_even_nodes, [=] __device__ (index_t idx) mutable {
    ...
    this->ExecuteNTasklets(total_even_nodes, [=] __device__ (index_t tid) mutable {
      index_t v = enode_queue_dview[enode_base_offset + tid];
  ...
  void XBlossomGpuEnginePerNode::ExpandAlternatingForest() {
    auto start_time = getNanoSecond();
  ...
  void XBlossomGpuEnginePerNode::TransformOddNodesInBlossom() {
    auto start_time = getNanoSecond();
    ...
    this->ExecuteNTaskletMax(total_even_nodes, [=] __device__ (index_t tid) mutable {
      index_t v = enode_queue_dview[enode_base_offset + tid];
    ...
    // Edge-level records per-edge counters from its prefix-sum scheduling.
    // Mirroring those counters here would require extra edge-count passes and
    // would charge node-level for work that is not part of its scheduling path.
