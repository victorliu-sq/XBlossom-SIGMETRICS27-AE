#ifndef XBLOSSOM_GPU_PAR_EDGE_H
#define XBLOSSOM_GPU_PAR_EDGE_H
...
namespace zblossom {
  class XBlossomGpuEngine2 : public SecondaryMaximumMatchingGPUEngine {
    friend class XBlossomGpuEngineTester;
    friend class XBlossomGpuEngineParEdgeTester;
    friend class XbsmGpuEngineTester<XBlossomGpuEngine2>;

  public:
    static inline String EngineName() {
      return "XBlossomGpuEngine2(ParEdge)";
    }

    XBlossomGpuEngine2() = delete;

    XBlossomGpuEngine2(const HGraph &g);

    XBlossomGpuEngine2(const HGraph &g, size_t path_table_buffer_ratio);
