...
namespace zblossom {
  class XBlossomGpuEngineHybrid2 : public SecondaryMaximumMatchingGPUEngine {
    // template<typename XBlossomEngine>
    friend class XbsmGpuEngineTester<XBlossomGpuEngineHybrid2>;

  public:
    static String EngineName() {
      return "XBlossomGpuEngineHybrid2(PerEdg+PerNode)";
    }

    XBlossomGpuEngineHybrid2() = delete;

    XBlossomGpuEngineHybrid2(const HGraph &g);

    XBlossomGpuEngineHybrid2(const HGraph &g, size_t path_table_buffer_ratio);
  ...
  private:
    ...
    const int LOAD_BALANCING_THRESHOLD_ = 50;
