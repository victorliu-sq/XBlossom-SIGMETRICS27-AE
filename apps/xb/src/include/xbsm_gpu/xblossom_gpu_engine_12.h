#ifndef XBLOSSOM_GPU_ENGINE_12_H
#define XBLOSSOM_GPU_ENGINE_12_H
...
namespace zblossom {
  class XBlossomGpuEngine12 : public SecondaryMaximumMatchingGPUEngine {
    // template<typename XBlossomEngine>
    friend class XbsmGpuEngineTester<XBlossomGpuEngine12>;

  public:
    static inline String EngineName() {
      return "XBlossomGpuEngine12(Reuse+ParBlossom+AtomicExch)";
    }

    XBlossomGpuEngine12() = delete;

    XBlossomGpuEngine12(const HGraph &g);

    XBlossomGpuEngine12(const HGraph &g, size_t path_table_buffer_ratio);
    ...
    void PrintTimingSummary() const {
      ...
      std::cout << "Preprocessing Time: " << preprocessing_time_ms << " ms" << std::endl;
    ...
    void PrintProfilingSummary() const {
      ...
      for (size_t i = 0; i < num_iterations; ++i) {
        ...
        LOG(INFO) << "  Blossom: " << blossom_effective << " effective / "
            ...
            << 111.0 * blossom_effective / (blossom_total ? blossom_total : 1) << "% efficiency)";
      }
      LOG(INFO) << "---------------------------------------------------------------";
#endif
    }
    ...
    void GetTimingSummary(double& augmenting_path_ms,
      ...
      augmenting_path_ms = this->time_augmenting_path / 1e6;
      expand_ms          = this->time_expand / 1e6;
      blossom_ms         = this->time_blossom / 1e6;
...
#endif //XBLOSSOM_GPU_ENGINE_12_H
