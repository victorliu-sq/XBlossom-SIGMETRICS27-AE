#ifndef XBLOSSOM_GPU_ENGINE_LOC_1_H
#define XBLOSSOM_GPU_ENGINE_LOC_1_H
...
namespace zblossom {
  class XBlossomGpuEngineLoc1 : public SecondaryMaximumMatchingGPUEngine {
    // template<typename XBlossomEngine>
    friend class XbsmGpuEngineTester<XBlossomGpuEngineLoc1>;

  public:
    static inline String EngineName() {
      return "XBlossomGpuEngineLoc1(Locality)";
    }

    XBlossomGpuEngineLoc1() = delete;

    XBlossomGpuEngineLoc1(const HGraph &g);
    ...
    void PrintTimingSummary() const {
      ...
      auto augmenting_path_time_ms = this->time_augmenting_path / 1e6;
      auto expand_and_blossom_time_ms = this->time_expand_and_blossom / 1e6;
      LOG(INFO) << "XBlossomGPUEngine Timing Summary:";
      LOG(INFO) << "Augmenting Path: " << augmenting_path_time_ms << " ms";
      LOG(INFO) << "Expand and Blossom: " << expand_and_blossom_time_ms << " ms";
      LOG(INFO) << "Total Time: " << augmenting_path_time_ms + expand_and_blossom_time_ms << " ms";
...
#endif //XBLOSSOM_GPU_ENGINE_LOC_1_H
