...
namespace zblossom {
  class XBlossomGpuEngineHybrid : public SecondaryMaximumMatchingGPUEngine {
    // template<typename XBlossomEngine>
    friend class XbsmGpuEngineTester<XBlossomGpuEngineHybrid>;

  public:
    static String EngineName() {
      return "XBlossomGpuEngineHybrid(PerEdg+PerNode)";
    }

    XBlossomGpuEngineHybrid() = delete;

    XBlossomGpuEngineHybrid(const HGraph &g);

    XBlossomGpuEngineHybrid(const HGraph &g, size_t path_table_buffer_ratio);
  ...
  private:
    ...
    const int LOAD_BALANCING_THRESHOLD_ = 500;
