#ifndef XBLOSSOM_GPU_ENGINE_8_H
#define XBLOSSOM_GPU_ENGINE_8_H
...
namespace zblossom {
  class XBlossomGpuEngine8 : public SecondaryMaximumMatchingGPUEngine {
    friend class XBlossomGpuEngineTester;
    friend class XBlossomGpuEngineParEdgeTester;
    friend class XbsmGpuEngineTester<XBlossomGpuEngine8>;

  public:
    static inline String EngineName() {
      return "XBlossomGpuEngine8(ReuseActiveTrees)";
    }

    XBlossomGpuEngine8() = delete;

    XBlossomGpuEngine8(const HGraph &g);

    XBlossomGpuEngine8(const HGraph &g, size_t path_table_buffer_ratio);
  ...
  private:
    ...
    // Lazy Reset
    // DArray<DFlag> is_root_and_reset_;
    // DArray<DFlag> is_re_add_;
    // DArray<index_t> prev_tree_roots_;
    // DArray<DFlag> prev_is_even_;
    ...
    inline bool IsExhausted() {
      ...
      if (this->exhausted_) {
        LOG(INFO) << "Enqueue is Empty. So No Aug path is found. Termiante this round. ";
      }
