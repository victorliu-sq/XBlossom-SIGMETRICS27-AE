#ifndef XBLOSSOM_GPU_ENGINE_7_H
#define XBLOSSOM_GPU_ENGINE_7_H

#include "max_match_engine.h"
// #include "enode_queue/enode_queue.h"
#include "enode_queue/enode_queue_2.h"
...
namespace zblossom {
  class XBlossomGpuEngine7 : public SecondaryMaximumMatchingGPUEngine {
    friend class XBlossomGpuEngineTester;
    friend class XBlossomGpuEngineParEdgeTester;
    friend class XbsmGpuEngineTester<XBlossomGpuEngine7>;

  public:
    static inline String EngineName() {
      return "XBlossomGpuEngine7(ExpandFirst)";
    }

    XBlossomGpuEngine7() = delete;

    XBlossomGpuEngine7(const HGraph &g);

    XBlossomGpuEngine7(const HGraph &g, size_t path_table_buffer_ratio);
