#ifndef XBLOSSOM_GPU_ENGINE_9_H
#define XBLOSSOM_GPU_ENGINE_9_H
...
namespace zblossom {
  class XBlossomGpuEngine9 : public SecondaryMaximumMatchingGPUEngine {
    // template<typename XBlossomEngine>
    friend class XbsmGpuEngineTester<XBlossomGpuEngine9>;

  public:
    static inline String EngineName() {
      return "XBlossomGpuEngine9(Reuse+ParBlossom)";
    }

    XBlossomGpuEngine9() = delete;

    XBlossomGpuEngine9(const HGraph &g);

    XBlossomGpuEngine9(const HGraph &g, size_t path_table_buffer_ratio);
