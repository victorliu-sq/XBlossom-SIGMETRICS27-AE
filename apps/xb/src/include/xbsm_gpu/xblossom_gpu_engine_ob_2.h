#ifndef XBLOSSOM_GPU_ENGINE_OB_2_H
#define XBLOSSOM_GPU_ENGINE_OB_2_H
...
namespace zblossom {
  class XBlossomGpuEngineOb2 : public SecondaryMaximumMatchingGPUEngine {
    // template<typename XBlossomEngine>
    friend class XbsmGpuEngineTester<XBlossomGpuEngineOb2>;

  public:
    static inline String EngineName() {
      return "XBlossomGpuEngineOb2(Locality)";
    }

    XBlossomGpuEngineOb2() = delete;

    XBlossomGpuEngineOb2(const HGraph &g);

    XBlossomGpuEngineOb2(const HGraph &g, size_t path_table_buffer_ratio);
...
#endif //XBLOSSOM_GPU_ENGINE_OB_2_H
