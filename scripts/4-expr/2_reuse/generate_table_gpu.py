...
if len(sys.argv) != 5:
    print("Usage: generate_table_gpu.py "
...
# Extract runtimes
avg_xb_pp_nr, ci_xb_pp_nr = extract_runtime_stats_seconds(timing_xb_pp_nr)
avg_xb_pp, ci_xb_pp = extract_runtime_stats_seconds(timing_xb_pp)
reused_tree_nodes_per_second, reset_tree_nodes_per_second, reuse_ratio, reuse_ratio_ci = extract_tree_reuse_metrics(
    timing_xb_pp, avg_xb_pp
)
...
print(
    f"[OK] {dataset}: "
    f"XB++NR={avg_xb_pp_nr:.6f}s ± {ci_xb_pp_nr:.6f}s, "
    f"XB++={avg_xb_pp:.6f}s ± {ci_xb_pp:.6f}s, "
