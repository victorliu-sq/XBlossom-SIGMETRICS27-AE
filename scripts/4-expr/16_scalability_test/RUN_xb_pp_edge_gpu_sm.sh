...
XB_PP_EDGE_BIN="${XB_BIN_DIR}/run_xb_pp_peredge_by_dataset"
METRICS_DIR="${PROJECT_DIR}/tmp/16_scalability_test/xb_pp_edge_gpu_sm"
RESULTS_DIR="${PROJECT_DIR}/results/16_scalability_test"
SUMMARY_CSV="${RESULTS_DIR}/xb_pp_edge_gpu_sm_scalability.csv"
ROUNDS="${ROUNDS:-20}"

GPU_SMS="${GPU_SMS:-188}"
GPU_SM_LIST="${GPU_SM_LIST:-1 2 4 8 16 32 64 96 128 188}"

require_executable "$XB_PP_EDGE_BIN" "XB++ edge-level binary"
rm -rf "$METRICS_DIR"
...
for entry in "${DATASET_XB_SCALABILITY_LIST[@]}"; do
  ...
  for max_cuda_sms in $GPU_SM_LIST; do
    ...
    TIMING_OUT="${METRICS_DIR}/xb_pp_edge_${DATASET}_${max_cuda_sms}sms.txt"
    : > "$TIMING_OUT"

    echo "Running XB++ edge-level on ${DATASET} with max ${max_cuda_sms}/${GPU_SMS} SMs (${ROUNDS} rounds)..."
    "$XB_PP_EDGE_BIN" \
        --dataset="$DATASET" \
        --rounds="$ROUNDS" \
