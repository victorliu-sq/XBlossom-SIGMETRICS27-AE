...
avg_runtime_s, runtime_ci_s = confidence_interval_95([v / 1000.0 for v in all_runtimes_ms])
avg_runtime_ms = avg_runtime_s * 1000.0
...
for idx, runtime_ms in enumerate(all_runtimes_ms, start=1):
    append_sample(
        samples_csv,
        ["Dataset", "Round", "AvgRuntime(s)"],
        {"Dataset": dataset, "Round": idx, "AvgRuntime(s)": f"{runtime_ms / 1000.0:.9f}"},
    )
