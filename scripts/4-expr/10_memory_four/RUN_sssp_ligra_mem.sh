...
METRICS_DIR="${PROJECT_DIR}/tmp/10_memory_four/sssp_ligra"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_sssp_ligra_mem_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/table_10_sssp_ligra.csv"
STAMP="${RESULTS_DIR}/.stamp.table_10_sssp_ligra"

if [[ -f "$STAMP" ]]; then
  echo "Table 10 SSSP Ligra part already generated (stamp found)"
  exit 0
fi
...
for entry in "${DATASET_SSSP_LIST[@]}"; do
  ...
  echo "Profiling Ligra-SSSP memory on ${DATASET} (${ROUNDS} rounds, symmetric=${SYMMETICS_TRUE}, src=${SRC})..."
