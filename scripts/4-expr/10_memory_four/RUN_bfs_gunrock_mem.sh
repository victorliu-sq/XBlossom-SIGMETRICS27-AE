...
METRICS_DIR="${PROJECT_DIR}/tmp/10_memory_four/bfs_gunrock"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_bfs_gunrock_mem_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/table_10_bfs_gunrock.csv"
STAMP="${RESULTS_DIR}/.stamp.table_10_bfs_gunrock"

if [[ -f "$STAMP" ]]; then
  echo "Table 10 BFS Gunrock part already generated (stamp found)"
  exit 0
fi
