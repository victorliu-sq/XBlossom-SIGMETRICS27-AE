...
METRICS_DIR="${PROJECT_DIR}/tmp/10_memory_four/sssp_gunrock"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_sssp_gunrock_mem_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/table_10_sssp_gunrock.csv"
STAMP="${RESULTS_DIR}/.stamp.table_10_sssp_gunrock"

if [[ -f "$STAMP" ]]; then
  echo "Table 10 SSSP Gunrock part already generated (stamp found)"
  exit 0
fi
