...
XB_PRO_BIN="${XB_BIN_DIR}/run_xb_pro_peredge_by_dataset"
METRICS_DIR="${PROJECT_DIR}/tmp/4_xb_pro_inst/edge"
RESULTS_DIR="${PROJECT_DIR}/results/4_xb_pro_inst"
STAMP="${RESULTS_DIR}/.stamp.figure_7_xbpro_edge"
ANALYZE_SCRIPT="${SCRIPT_DIR}/analyze_xb_pro_inst_edge.sh"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_xb_pro_edge_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/figure_7_xb_pro_edge.csv"
SAMPLES_CSV="${RESULTS_DIR}/figure_7_xb_pro_edge_samples.csv"
...
if [[ -z "$XB_PRO_ONLY_DATASETS" && -f "$STAMP" ]]; then
  echo "Figure 7 XB-Pro edge part already generated (stamp found)"
  exit 0
fi

require_executable "$XB_PRO_BIN" "XB-Pro per-edge binary"
rm -rf "$METRICS_DIR"
...
for entry in "${DATASET_CSR_LIST[@]}"; do
  ...
  for ITERATION in $(seq 1 "$ITERATIONS"); do
    TIMING_OUT="${METRICS_DIR}/xb_pro_edge_${DATASET}_iter_${ITERATION}_timing.txt"
    PROFILER_OUT="${METRICS_DIR}/xb_pro_edge_${DATASET}_iter_${ITERATION}_profiling.txt"
    : > "$TIMING_OUT"
    : > "$PROFILER_OUT"

    echo "Profiling XB-Pro edge ${DATASET} iteration ${ITERATION}/${ITERATIONS} (${ROUNDS} rounds, ${CURRENT_XB_PRO_THREADS} threads)..."
