...
if runtime_groups:
    misses_per_group = total_llc_misses / len(runtime_groups)
    for runtime_group_s in runtime_groups:
        append_sample(
            samples_csv,
            ["Dataset", "LLCMissRate(%)", "LLCMissFreq(GMiss/s)"],
            {
                "Dataset": dataset,
                "LLCMissRate(%)": f"{llc_miss_rate:.9f}",
                "LLCMissFreq(GMiss/s)": f"{misses_per_group / runtime_group_s / 1e9:.9f}",
            },
        )
else:
    append_sample(
        ...
        ["Dataset", "LLCMissRate(%)", "LLCMissFreq(GMiss/s)"],
        {
            "Dataset": dataset,
            "LLCMissRate(%)": f"{llc_miss_rate:.9f}",
            "LLCMissFreq(GMiss/s)": f"{llc_miss_freq:.9f}",
        },
    )
rates = read_grouped_samples(samples_csv, value_col="LLCMissRate(%)")
freqs = read_grouped_samples(samples_csv, value_col="LLCMissFreq(GMiss/s)")
with open(summary_csv, "w", newline="") as f:
    ...
    for sample_dataset, rate_values in rates.items():
        rate_mean, rate_ci = confidence_interval_95(rate_values)
        freq_mean, freq_ci = confidence_interval_95(freqs.get(sample_dataset, []))
        writer.writerow([
            sample_dataset,
            f"{rate_mean:.6f}",
            f"{rate_ci:.6f}",
