...
parser = argparse.ArgumentParser(description="Plot throughput for XB plus BFS/SSSP/MSSP")
...
def read_throughput_map(path):
    df = pd.read_csv(path)
    return {
        normalize(row["Dataset"]): float(row["Throughput(MEdges/s)"]) / 1000.0
...
xb_pro = values(read_throughput_map(args.xb_pro_csv), datasets)
xb_pp = values(read_throughput_map(args.xb_pp_csv), datasets)
pairs = [
    ("X-Blossom", "XB-Pro", "XB++", xb_pro, xb_pp),
    ("BFS", "Ligra", "Gunrock", values(read_throughput_map(args.bfs_ligra_csv), datasets), values(read_throughput_map(args.bfs_gunrock_csv), datasets)),
    ("SSSP", "Ligra", "Gunrock", values(read_throughput_map(args.sssp_ligra_csv), datasets), values(read_throughput_map(args.sssp_gunrock_csv), datasets)),
    ("MSSP", "Ligra", "Gunrock", values(read_throughput_map(args.multisssp_ligra_csv), datasets), values(read_throughput_map(args.multisssp_gunrock_csv), datasets)),
]

plt.rcParams.update({"font.size": 6, "axes.linewidth": 1.0})
...
for row, (row_label, cpu_label, gpu_label, cpu, gpu) in enumerate(pairs):
    ...
    ymax = positive_max(cpu, gpu) * 4.0
    for col, dataset in enumerate(datasets):
        ax = axes[row, col]
        vals = [cpu[col], gpu[col]]
        ax.bar(bar_x, vals, width=bar_width, color=colors, edgecolor="black")
        if np.isfinite(vals[0]) and np.isfinite(vals[1]) and vals[0] > 0:
            ratio = vals[1] / vals[0]
            ax.text(bar_x[1], vals[1] * 1.12, f"{ratio:.2g}x",
            ...
            ax.text(bar_x[1], ymin * 1.2, "n/a", ha="center", va="bottom", fontsize=5)

        if row == 0:
            ax.set_title(dataset, fontsize=6)
        ax.set_xticks(tick_x, [cpu_label, gpu_label])
        ax.tick_params(axis="x", labelsize=4.5, pad=1)
        ax.set_xlim(-0.6, 0.6)
        ax.set_yscale("log")
        ...
        if col == 0:
            ax.set_ylabel("Throughput (GEdges/s)", fontsize=5, labelpad=1)
    ...
    fig.text(0.975, row_center, row_label,
             rotation=90, va="center", ha="left", fontsize=7, fontweight="bold")
