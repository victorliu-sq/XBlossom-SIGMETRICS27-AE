...
parser = argparse.ArgumentParser(description="Plot runtime for XB plus BFS/BC/SSSP")
...
pairs = [
    ...
    ("BC", "Ligra", "Gunrock", values(read_csv_value_map(args.bc_ligra_csv), datasets), values(read_csv_value_map(args.bc_gunrock_csv), datasets)),
...
for row, (row_label, cpu_label, gpu_label, cpu, gpu) in enumerate(pairs):
    ...
    fig.text(0.975, 1.0 - (row + 0.5) / len(pairs), row_label,
