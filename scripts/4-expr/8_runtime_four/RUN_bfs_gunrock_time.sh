...
METRICS_DIR="${PROJECT_DIR}/tmp/8_runtime_four/bfs_gunrock"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_bfs_gunrock_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/figure_8_bfs_gunrock.csv"
STAMP="${RESULTS_DIR}/.stamp.figure_8_bfs_gunrock"

if [[ -f "$STAMP" ]]; then
  echo "Figure 8 BFS Gunrock part already generated (stamp found)"
  exit 0
fi
