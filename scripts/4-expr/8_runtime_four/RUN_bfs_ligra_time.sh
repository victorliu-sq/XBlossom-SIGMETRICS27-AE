...
METRICS_DIR="${PROJECT_DIR}/tmp/8_runtime_four/bfs_ligra"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_bfs_ligra_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/figure_8_bfs_ligra.csv"
STAMP="${RESULTS_DIR}/.stamp.figure_8_bfs_ligra"

if [[ -f "$STAMP" ]]; then
  echo "Figure 8 BFS Ligra part already generated (stamp found)"
  exit 0
fi
...
cp "${SUMMARY_CSV_TEMP%.csv}_samples.csv" "${RESULTS_DIR}/figure_8_bfs_ligra_samples.csv"
touch "$STAMP"
