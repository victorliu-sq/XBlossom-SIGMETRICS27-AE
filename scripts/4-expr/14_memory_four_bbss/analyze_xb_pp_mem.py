...
# ============================================================
if len(profiles) == 1:
    total_bytes = aggregate_metric(profiles[0]) * (rounds / profile_rounds)  # raw dram bytes
else:
    total_bytes = sum(aggregate_metric(profile) for profile in profiles)
...
effective_memory_bandwidth_gbps   = total_bytes / runtime / 1e9  # GB/s
...
if len(profiles) > 1 and runtime_groups:
    ...
    for profile, runtime_group_s in zip(profiles, runtime_groups):
        group_bytes = aggregate_metric(profile) * (group_rounds / profile_rounds)
        append_sample(
            samples_csv,
            ["Dataset", "EffectiveMemoryBandwidth(GB/s)"],
            {
                "Dataset": dataset,
                "EffectiveMemoryBandwidth(GB/s)": f"{group_bytes / runtime_group_s / 1e9:.9f}",
            },
        )
elif runtime_groups:
    bytes_per_group = total_bytes / len(runtime_groups)
    for runtime_group_s in runtime_groups:
        append_sample(
            ...
            {
                "Dataset": dataset,
                "EffectiveMemoryBandwidth(GB/s)": f"{bytes_per_group / runtime_group_s / 1e9:.9f}",
            },
        )
else:
    append_sample(
        samples_csv,
        ["Dataset", "EffectiveMemoryBandwidth(GB/s)"],
        {"Dataset": dataset, "EffectiveMemoryBandwidth(GB/s)": f"{effective_memory_bandwidth_gbps:.9f}"},
    )
rewrite_summary_from_samples(
    samples_csv,
    summary_out,
...
print(f"  Data Load Rate:   {effective_memory_bandwidth_gbps:.3f} GB/s\n")
