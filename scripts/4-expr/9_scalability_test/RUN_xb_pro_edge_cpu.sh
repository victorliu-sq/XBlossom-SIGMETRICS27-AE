...
XB_PRO_EDGE_BIN="${XB_BIN_DIR}/run_xb_pro_peredge_by_dataset"
METRICS_DIR="${PROJECT_DIR}/tmp/9_scalability_test/xb_pro_edge_cpu"
RESULTS_DIR="${PROJECT_DIR}/results/9_scalability_test"
SUMMARY_CSV="${RESULTS_DIR}/xb_pro_edge_cpu_scalability.csv"
ROUNDS="${ROUNDS:-20}"
THREAD_LIST="${THREAD_LIST:-1 2 4 8 16 32 48}"
XB_PRO_ONLY_DATASETS="${XB_PRO_ONLY_DATASETS:-}"

require_executable "$XB_PRO_EDGE_BIN" "XB-Pro edge-level binary"
rm -rf "$METRICS_DIR"
mkdir -p "$METRICS_DIR" "$RESULTS_DIR"

if [[ -n "$XB_PRO_ONLY_DATASETS" && -f "$SUMMARY_CSV" ]]; then
  SUMMARY_CSV_TEMP="${METRICS_DIR}/_xb_pro_edge_cpu_scalability.csv"
...
for entry in "${DATASET_LIST[@]}"; do
  ...
  for threads in $THREAD_LIST; do
    TIMING_OUT="${METRICS_DIR}/xb_pro_edge_${DATASET}_${threads}t.txt"
    : > "$TIMING_OUT"

    echo "Running XB-Pro edge-level on ${DATASET} with ${threads} CPU threads (${ROUNDS} rounds)..."
    "$XB_PRO_EDGE_BIN" \
        --dataset="$DATASET" \
        --rounds="$ROUNDS" \
