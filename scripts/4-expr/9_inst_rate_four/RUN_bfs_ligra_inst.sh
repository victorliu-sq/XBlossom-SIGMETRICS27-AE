...
METRICS_DIR="${PROJECT_DIR}/tmp/9_inst_rate_four/bfs_ligra"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_bfs_ligra_inst_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/table_9_bfs_ligra.csv"
STAMP="${RESULTS_DIR}/.stamp.table_9_bfs_ligra"

if [[ -f "$STAMP" ]]; then
  echo "Table 9 BFS Ligra part already generated (stamp found)"
  exit 0
fi
...
for entry in "${DATASET_BFS_LIST[@]}"; do
  ...
  echo "Profiling Ligra-BFS instructions on ${DATASET} (${ROUNDS} rounds, symmetric=${SYMMETICS_TRUE}, src=${SRC})..."
