...
METRICS_DIR="${PROJECT_DIR}/tmp/9_inst_rate_four/bfs_gunrock"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_bfs_gunrock_inst_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/table_9_bfs_gunrock.csv"
STAMP="${RESULTS_DIR}/.stamp.table_9_bfs_gunrock"

if [[ -f "$STAMP" ]]; then
  echo "Table 9 BFS Gunrock part already generated (stamp found)"
  exit 0
fi
...
for entry in "${DATASET_BFS_LIST[@]}"; do
  ...
  echo "Profiling Gunrock-BFS on ${DATASET} (src=${SRC})..."
