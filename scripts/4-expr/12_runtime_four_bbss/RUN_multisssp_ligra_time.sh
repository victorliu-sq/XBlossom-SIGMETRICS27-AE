...
METRICS_DIR="${PROJECT_DIR}/tmp/12_runtime_four_bbss/multisssp_ligra"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_multisssp_ligra_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/figure_12_multisssp_ligra.csv"
SAMPLES_CSV="${RESULTS_DIR}/figure_12_multisssp_ligra_samples.csv"
STAMP="${RESULTS_DIR}/.stamp.figure_12_multisssp_ligra"

if [[ -f "$STAMP" ]]; then
  echo "Figure 12 MultiSSSP Ligra part already generated (stamp found)"
  exit 0
fi
