...
METRICS_DIR="${PROJECT_DIR}/tmp/13_inst_rate_four_bbss/sssp_ligra"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_sssp_ligra_inst_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/table_13_sssp_ligra.csv"
STAMP="${RESULTS_DIR}/.stamp.table_13_sssp_ligra"

if [[ -f "$STAMP" ]]; then
  echo "Table 13 SSSP Ligra part already generated (stamp found)"
  exit 0
fi
...
for entry in "${DATASET_SSSP_LIST[@]}"; do
  ...
  for ITERATION in $(seq 1 "$ITERATIONS"); do
    ...
    echo "Profiling Ligra-SSSP instructions on ${DATASET} iteration ${ITERATION}/${ITERATIONS} (${ROUNDS} rounds, symmetric=${SYMMETICS_TRUE}, src=${SRC})..."
...
cp "${SUMMARY_CSV_TEMP%.csv}_samples.csv" "${RESULTS_DIR}/table_13_sssp_ligra_samples.csv"
touch "$STAMP"
