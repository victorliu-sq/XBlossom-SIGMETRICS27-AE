...
METRICS_DIR="${PROJECT_DIR}/tmp/13_inst_rate_four_bbss/xb_pro"
RESULTS_DIR="${PROJECT_DIR}/results/13_inst_rate_four_bbss"
STAMP="${RESULTS_DIR}/.stamp.table_13_xbpro"
ANALYZE_SCRIPT="${SCRIPT_DIR}/analyze_xb_pro_inst.sh"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_xb_pro_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/table_13_xb_pro.csv"
SAMPLES_CSV="${RESULTS_DIR}/table_13_xb_pro_samples.csv"
ROUNDS="${ROUNDS:-20}"
...
if [[ -z "$XB_PRO_ONLY_DATASETS" && -f "$STAMP" ]]; then
  echo "Table 13 XB-Pro part already generated (stamp found)"
  exit 0
fi
...
SUMMARY_SAMPLES_CSV="${SUMMARY_CSV_TEMP%.csv}_samples.csv"
seed_csv_excluding_datasets "$SAMPLES_CSV" "$SUMMARY_SAMPLES_CSV" "$XB_PRO_ONLY_DATASETS"
...
for entry in "${DATASET_CSR_LIST[@]}"; do
  ...
  for ITERATION in $(seq 1 "$ITERATIONS"); do
    ...
    echo "Profiling XB-Pro ${DATASET} iteration ${ITERATION}/${ITERATIONS} (${ROUNDS} rounds, ${CURRENT_XB_PRO_THREADS} threads)..."
