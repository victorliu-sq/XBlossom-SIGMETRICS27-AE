...
runtime_groups = grouped_runtime_sums(round_runtimes_s, groups=20)
if runtime_groups:
    ...
    for runtime_group_s in runtime_groups:
        append_sample(
            ...
            {
                "Dataset": dataset,
                "InstructionExecRate(GIPS)": f"{instructions_per_group / runtime_group_s / 1e9:.9f}",
            },
        )
else:
    append_sample(
        samples_csv,
        ["Dataset", "InstructionExecRate(GIPS)"],
        {"Dataset": dataset, "InstructionExecRate(GIPS)": f"{inst_exec_rate_gips:.9f}"},
    )
