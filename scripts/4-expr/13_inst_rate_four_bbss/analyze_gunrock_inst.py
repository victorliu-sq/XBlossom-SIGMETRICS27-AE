...
# ============================================================
if len(sys.argv) != 5:
    print("Usage: analyze_gunrock_inst.py <dataset> <timing_file> <profiling_file_or_dir> <summary_out>")
    sys.exit(1)
...
for idx, profile in enumerate(profiles):
    ...
    if runtime_s <= 0:
        continue
    total_insts = aggregate_metric(profile)
    sample_rates.append(total_insts / runtime_s / 1e9)
...
for sample_rate in sample_rates:
    append_sample(
        ...
        {"Dataset": dataset, "InstructionRate(GIPS)": f"{sample_rate:.9f}"},
    )
...
if sample_rates:
    print(f"  Instruction Rate: {sum(sample_rates) / len(sample_rates):.3f} GIPS")
