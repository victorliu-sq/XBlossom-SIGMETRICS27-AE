...
METRICS_DIR="${PROJECT_DIR}/tmp/13_inst_rate_four_bbss/xb_pp"
RESULTS_DIR="${PROJECT_DIR}/results/13_inst_rate_four_bbss"
STAMP="${RESULTS_DIR}/.stamp.table_13_xbpp"
ANALYZE_SCRIPT="${SCRIPT_DIR}/analyze_xb_pp_inst.sh"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_xb_pp_inst_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/table_13_xb_pp.csv"
SAMPLES_CSV="${RESULTS_DIR}/table_13_xb_pp_samples.csv"
...
if [[ -f "$STAMP" ]]; then
  echo "Table 13 XB-PP part already generated (stamp found)"
  exit 0
fi
...
for entry in "${DATASET_CSR_LIST[@]}"; do
  ...
  echo "Running XB++ ${DATASET} instruction-rate timing (${ROUNDS} rounds as ${TIMING_CHUNKS}x${CHUNK_ROUNDS})..."
  ...
  echo "Profiling XB++ ${DATASET} instruction counter (${PROFILE_RUNS} profile runs, ${PROFILE_ROUNDS} rounds each)..."
