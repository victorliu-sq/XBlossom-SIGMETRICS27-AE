...
METRICS_DIR="${PROJECT_DIR}/tmp/13_inst_rate_four_bbss/sssp_gunrock"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_sssp_gunrock_inst_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/table_13_sssp_gunrock.csv"
STAMP="${RESULTS_DIR}/.stamp.table_13_sssp_gunrock"

if [[ -f "$STAMP" ]]; then
  echo "Table 13 SSSP Gunrock part already generated (stamp found)"
  exit 0
fi
