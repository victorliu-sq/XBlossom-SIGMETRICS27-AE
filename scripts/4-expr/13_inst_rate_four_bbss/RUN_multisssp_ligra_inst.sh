...
RESULTS_DIR="${PROJECT_DIR}/results/13_inst_rate_four_bbss"
ANALYZE_SCRIPT="${SCRIPT_DIR}/analyze_ligra_inst.sh"
ROUNDS="${ROUNDS:-200}"
ITERATIONS="${ITERATIONS:-5}"
SYMMETICS_TRUE=1

METRICS_DIR="${PROJECT_DIR}/tmp/13_inst_rate_four_bbss/multisssp_ligra"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_multisssp_ligra_inst_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/table_13_multisssp_ligra.csv"
STAMP="${RESULTS_DIR}/.stamp.table_13_multisssp_ligra"

if [[ -f "$STAMP" ]]; then
  echo "Table 13 MultiSSSP Ligra part already generated (stamp found)"
  exit 0
fi
...
for entry in "${DATASET_MULTISSSP_LIST[@]}"; do
  ...
  for ITERATION in $(seq 1 "$ITERATIONS"); do
    ...
    echo "Profiling Ligra-MultiSSSP instructions on ${DATASET} iteration ${ITERATION}/${ITERATIONS} (${ROUNDS} rounds, symmetric=${SYMMETICS_TRUE}, src=${SRC})..."
...
cp "${SUMMARY_CSV_TEMP%.csv}_samples.csv" "${RESULTS_DIR}/table_13_multisssp_ligra_samples.csv"
touch "$STAMP"
