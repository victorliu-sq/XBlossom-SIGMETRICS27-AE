...
METRICS_DIR="${PROJECT_DIR}/tmp/11_throughput/bc_ligra"
RESULTS_DIR="${PROJECT_DIR}/results/11_throughput"
STAMP="${RESULTS_DIR}/.stamp.throughput_bc_ligra"
ANALYZE_SCRIPT="${SCRIPT_DIR}/analyze_throughput.sh"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_bc_ligra_throughput_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/throughput_bc_ligra.csv"
...
if [[ -f "$STAMP" ]]; then
  echo "Experiment 11 Ligra-BC throughput already generated (stamp found)"
  exit 0
fi

require_executable "$LIGRA_BC_THROUGHPUT_BIN" "Ligra BC throughput binary"
rm -rf "$METRICS_DIR"
...
for entry in "${DATASET_BFS_LIST[@]}"; do
  ...
  TIMING_OUT="${METRICS_DIR}/bc_ligra_${DATASET}_throughput.txt"
  : > "$TIMING_OUT"

  echo "Running Ligra-BC throughput on ${DATASET} (${ROUNDS} rounds, symmetric=${SYMMETICS_TRUE}, src=${SRC})..."
  "$LIGRA_BC_THROUGHPUT_BIN" -rounds "$ROUNDS" -r "$SRC" -s "$LIGRA_GRAPH" | tee -a "$TIMING_OUT"
