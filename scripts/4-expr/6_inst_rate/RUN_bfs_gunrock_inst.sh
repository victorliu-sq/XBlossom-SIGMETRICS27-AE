...
METRICS_DIR="${PROJECT_DIR}/tmp/6_inst_rate/bfs_gunrock"
RESULTS_DIR="${PROJECT_DIR}/results/6_inst_rate"
STAMP="${RESULTS_DIR}/.stamp.table_5_gunrock"
ANALYZE_SCRIPT="${SCRIPT_DIR}/analyze_bfs_gunrock_inst.sh"
SUMMARY_CSV_TEMP="${METRICS_DIR}/_bfs_gunrock_inst_summary.csv"
SUMMARY_CSV="${RESULTS_DIR}/table_5_bfs_gunrock.csv"
NCU_METRIC="sm__sass_thread_inst_executed.sum"

if [[ -f "$STAMP" ]]; then
  echo "Table 5 Gunrock part already generated (stamp found)"
  exit 0
fi
