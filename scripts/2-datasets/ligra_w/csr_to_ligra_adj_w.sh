...
OUTPUT_DIR="${PROJECT_DIR}/data/ligra_w"
STAMP="${OUTPUT_DIR}/.stamp.csr_to_ligra_w"
CONDA_ENV_NAME="${CONDA_ENV_NAME:-graph-env}"

mkdir -p "${OUTPUT_DIR}"

if [[ -f "$STAMP" ]]; then
  echo "Weighted Ligra SSSP conversion already completed."
...
for entry in "${DATASETS[@]}"; do
  ...
  echo "------------------------------------------------------------"
  echo "Processing weighted Ligra SSSP graph:"
...
touch "$STAMP"
echo "All weighted Ligra SSSP datasets processed successfully."
